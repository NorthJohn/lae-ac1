...
def readWrite(n, row, doWrite, value, timeout):
  ...
  try :
    if doWrite:  # write operation
      abc = instr.write_register(address, value, number_of_decimals=scale, functioncode=6, signed=True)
      logging.info(f'{logText} =>{value:4}  {units}')

    else:             # read operation
      rvalue = instr.read_register(address, number_of_decimals=scale, functioncode=3, signed=True)
      success = ''
      if value :
...
if __name__ == "__main__":
  ...
  parser.add_argument("-t", "--timeout", help="serial port transaction timeout and sleep following an exception, default is %(default).1f secs", type=float, default=1.0)
  ...
  try :
    ...
    bytesize=7, timeout=args.timeout, exclusive=args.lock, stopbits=1) as serial0:
      ...
      if args.all :
        ...
        if not len(args.register):
          logging.warning('Specify one or more registers or --all') 
          sys.exit(0)
